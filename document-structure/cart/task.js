const quantityControl_d = document.querySelectorAll(".product__quantity-control_dec");
const quantityControl_i = document.querySelectorAll(".product__quantity-control_inc");
const buttons = document.querySelectorAll(".product__add");
let quantityValue;

//действие на убавить к-во
function dec(control) {
  quantityValue = control.parentElement.querySelector(".product__quantity-value");
  let currentValue = parseInt(quantityValue.textContent);
  if (currentValue > 1) {quantityValue.textContent = currentValue - 1;}
}
